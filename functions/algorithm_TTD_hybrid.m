function [R_convergence] = algorithm_TTD_hybrid(para, h, P, user_r, user_theta)
%Penalty-based algorithm for hybrid configurations
...
function [A, t] = update_analog(para, P, D, t)
    ...
    t_cum = t;
    t_cum(1:para.N_T/2,:) = cumsum(t_cum(1:para.N_T/2,:), 1,'forward');
    t_cum(para.N_T/2+1:end,:) = cumsum(t_cum(para.N_T/2+1:end,:), 1,'reverse');
    % update PSs
    ...
    for n = 1:para.N_RF
        ...
        for q_ = para.N_T:-1:para.N_T/2+1
            if q_ < para.N_T
...
function [Am] = analog_bamformer(para, A, t, fm)
    t(1:para.N_T/2,:) = cumsum(t(1:para.N_T/2,:), 1,'forward');
    t(para.N_T/2+1:end,:) = cumsum(t(para.N_T/2+1:end,:), 1,'reverse');
...
function [a, t_fs] = initialize_analog(para, theta, r)
    c = 3e8;
    N_sub = para.N/para.N_T;
    ...
    t_fs = zeros(para.N_T, 1);
    for q = 2:para.N_T
        delta_t = t_inf(q) - t_inf(q-1);
        if delta_t >=0
            t_fs(q) = min([delta_t, para.t_max]);
        else
            t_fs(q) = 0;
        end
    end

    t_rs = zeros(para.N_T, 1);
    t_inf = flip(t_inf);
    for q = 2:para.N_T
        delta_t = t_inf(q) - t_inf(q-1);
        if delta_t >=0
            t_rs(q) = min([delta_t, para.t_max]);
        else
            t_rs(q) = 0;
        end
    end
